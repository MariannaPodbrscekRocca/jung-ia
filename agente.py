"""
================================================================================
PROYECTO: Jung.IA - Agente de Reclutamiento Interactivo (Versión Maestra Completa)
...
PROPÓSITO GENERAL DEL SISTEMA:
  Este sistema interactivo integra un entorno visual en Gradio con la lógica 
  robusta en consola CLI para automatizar la primera etapa del proceso de 
  selección para Jung Tech Company usando el modelo de las 16 personalidades 
  de Carl Jung y el MBTI.
================================================================================
"""
...
TEXTOS = {
    ...
    "ENG": {
        ...
        "arquetipo_intro": (
            ...
            "{nombre}, chose {personaje} (MBTI: {mbti})."
        ),
...
# ------------------------------------------------------------------------------
# 2. FUNCIONES UTILITARIAS Y DE CORREO REAL
# ------------------------------------------------------------------------------
        ...
        if idioma == "ESP":
            ...
            <html>
              <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6;">
                <h2 style="color: #2c3e50;">Jung Tech Company - Proceso de Selección</h2>
                <p>Hola <b>{nombre}</b>,</p>
                <p>Gracias por completar la primera etapa de selección con nuestro agente inteligente <b>Jung.IA</b>.</p>
                <div style="background-color: #f8f9fa; border-left: 4px solid #007bff; padding: 15px; margin: 20px 0;">
                  <h3>Resumen de tu Evaluación:</h3>
                  <pre style="font-family: inherit; white-space: pre-wrap;">{resumen_texto}</pre>
                </div>
                <h3>Fechas y Horas Tentativas para Entrevista (Agosto 2026):</h3>
                <p>Por favor, <b>responde directamente a este correo</b> indicándonos cuál de las siguientes opciones se acomoda mejor a tu agenda:</p>
                <ul>
                  {fechas_html}
                </ul>
                <br>
                <p>Atentamente,</p>
                <p><b>Equipo de Reclutamiento & Jung.IA</b><br><i>Jung Tech Company</i></p>
              </body>
            </html>
            """
        else:
            ...
            <html>
              <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6;">
                <h2 style="color: #2c3e50;">Jung Tech Company - Recruitment Process</h2>
                <p>Hello <b>{nombre}</b>,</p>
                <p>Thank you for completing stage 1 with our AI agent <b>Jung.IA</b>.</p>
                <div style="background-color: #f8f9fa; border-left: 4px solid #007bff; padding: 15px; margin: 20px 0;">
                  <h3>Your Assessment Summary:</h3>
                  <pre style="font-family: inherit; white-space: pre-wrap;">{resumen_texto}</pre>
                </div>
                <h3>Tentative Interview Options (August 2026):</h3>
                <p>Please <b>reply directly to this email</b> letting us know which date works best for you:</p>
                <ul>
                  {fechas_html}
                </ul>
                <br>
                <p>Best regards,</p>
                <p><b>Recruitment Team & Jung.IA</b><br><i>Jung Tech Company</i></p>
              </body>
            </html>
            """
...
def mostrar_cabecera(serie=None, nombre_usuario=None, idioma="ESP", genero="N"):
    print("=======================================================================")
    if idioma == "ENG":
        print("  Stage 1 of the Selection Process for the Tech Company Jung Tech Company  ")
...
# ------------------------------------------------------------------------------
# 3. CARGA DE LA MATRIZ JUNGUIANA DE EXACTAMENTE 16 PERSONALIDADES
# ------------------------------------------------------------------------------
def cargar_matriz():
    try:
...
# ------------------------------------------------------------------------------
# RESPUESTA DINÁMICA DE IA / FALLBACK ROTATIVO
# ------------------------------------------------------------------------------
def respuesta_ia_pregunta_adicional(tipo_pregunta, nombre_funcion, codigo_funcion, nombre_call, genero_actual, resp_idioma, mbti_val, area_ti, resp_usadas):
    ...
    if OPENAI_AVAILABLE and OPENAI_API_KEY:
        try:
            ...
            sys_prompt = "Eres Jung.IA, la musa de reclutamiento de Jung Tech Company. Explica brevemente (máximo 2 oraciones) y con tono inspirador la utilidad práctica de esta función."
    ...
    while True:
        ...
        if any(w in op_low for w in ['dato', 'correo', 'email', 'telefono', 'teléfono', 'contacto', 'excel', 'verificar']):
            ...
            respuesta_ia = respuesta_ia_pregunta_adicional(
                "GENERAL", "General", "Jung", nombre_call, genero_actual, resp_idioma, mbti_val, area_ti, resp_usadas
            )
...
# ------------------------------------------------------------------------------
# 4. BUCLE PRINCIPAL Y NAVEGACIÓN EN CONSOLA (CLI)
# ------------------------------------------------------------------------------
def ejecutar_agente():
            ...
            while True:
                ...
                while True:
                    ...
                    if elec_aux == "SABER_EXCEL":
                        ...
                        resp_ia = respuesta_ia_pregunta_adicional("AUXILIAR", f_aux, "Auxiliar", nombre_call, genero_actual, resp_idioma, mbti_val, area_ti, resp_usadas_ia)
                        print(f"\nJung.IA: {resp_ia}")
                    elif elec_aux == "PASAR":
                        break

                # BUCLE FUNCIÓN TERCIARIA
                ...
                while True:
                    ...
                    if elec_ter == "SABER_EXCEL":
                        ...
                        resp_ia = respuesta_ia_pregunta_adicional("TERCIARIA", f_ter, "Terciaria", nombre_call, genero_actual, resp_idioma, mbti_val, area_ti, resp_usadas_ia)
                        print(f"\nJung.IA: {resp_ia}")
                    elif elec_ter == "PASAR":
                        break

                # BUCLE FUNCIÓN INFERIOR
                ...
                while True:
                    ...
                    if elec_inf == "SABER_EXCEL":
                        ...
                        resp_ia = respuesta_ia_pregunta_adicional("INFERIOR", f_inf, "Inferior", nombre_call, genero_actual, resp_idioma, mbti_val, area_ti, resp_usadas_ia)
                        print(f"\nJung.IA: {resp_ia}")
                    elif elec_inf == "PASAR":
                        break

                # BUCLE PATRÓN DE LOOP
                ...
                while True:
                    ...
                    if elec_loop == "SABER_EXCEL":
                        ...
                        resp_ia = respuesta_ia_pregunta_adicional("LOOP", val_loop, "Loop", nombre_call, genero_actual, resp_idioma, mbti_val, area_ti, resp_usadas_ia)
                        print(f"\nJung.IA: {resp_ia}")
                ...
                if resp_idioma == "ENG":
                    col_desc = 'arquetipos_DESC_ENG' if 'arquetipos_DESC_ENG' in fila_datos else 'arquetipos_DESC_ESP.1'
                    arq_desc = fila_datos.get(col_desc, str(fila_datos.get('arquetipos_DESC_ESP', '')))
                else:
                ...
                resumen_correo = (
                    f"Candidato: {datos_usr['nombres']} {datos_usr['apellidos']}\n"
                    f"Personaje Seleccionado: {char_nombre}\n"
                    f"Tipo MBTI: {mbti_val}\n"
                    f"Arquetipo Profesional: {arq_val}\n"
                    f"Área TI Asignada: {area_ti}\n"
                    f"Razón: {razon_ti}"
                )
                
                enviado_ok = enviar_correo_real(datos_usr['correo'], nombre_call, resumen_correo, fechas_tentativas, resp_idioma)
                ...
                if resp_idioma == "ESP":
                    print("\n[!] Por favor, responde a ese correo indicando cuál de estas fechas y horas te acomoda mejor para tu entrevista.")
                else:
                    print("\n[!] Please reply to that email indicating which date and time works best for your interview.")
...
# ==============================================================================
# 5. INTERFAZ GRÁFICA WEB CON GRADIO (MODO VISUAL CON BOTONES LIMPIOS)
# ==============================================================================
if os.path.exists(CSV_PATH):
    df_matriz_web = pd.read_csv(CSV_PATH)
else:
    df_matriz_web = None

def validar_registro_web(nombre, apellido, correo):
    if not nombre.strip() or not apellido.strip():
        return "⚠️ Por favor, escribe tu nombre y apellido para comenzar.", gr.update(visible=True), gr.update(visible=False), ""
    mensaje_cabecera = f"🎪 ¡Bienvenido/a **{nombre} {apellido}** a Jung.IA! 🎪\nHaz clic en el personaje con el que más te identifiques:"
    return "", gr.update(visible=False), gr.update(visible=True), mensaje_cabecera

def evaluar_seleccion_web(nombre_personaje, mbti_personaje, usuario):
    info_matriz = ""
    if df_matriz_web is not None:
        cols = [c for c in df_matriz_web.columns if 'mbti' in c.lower()]
        if cols:
            match = df_matriz_web[df_matriz_web[cols[0]].astype(str).str.upper() == mbti_personaje.upper()]
            if not match.empty:
                info_matriz = f"\n\n📊 **Datos extraídos de tu Matriz:**\n`{match.iloc[0].to_dict()}`"

    resultado_final = f"""
    🎪 **¡EVALUACIÓN COMPLETADA EN JUNG.IA!** 🎪
    --------------------------------------------------
    👤 **Candidato:** {usuario}
    🎭 **Personaje Seleccionado:** {nombre_personaje}
    ⚡ **Arquetipo Revelado (MBTI):** {mbti_personaje}
    
    🧠 **Diagnóstico Psicométrico (Jung.IA):**
    Tu elección refleja un perfil cognitivo alineado a la estructura de {nombre_personaje}. Muestras alta versatilidad y pensamiento analítico bajo entornos de resolución estratégica.
    {info_matriz}
    """
    return resultado_final, gr.update(visible=False), gr.update(visible=True)

with gr.Blocks(css="styles.css", title="Jung.IA - Circo Digital") as demo:
    
    state_usuario = gr.State("")

    gr.Markdown("# 🎪 JUNG.IA: EL CIRCO DIGITAL DE LAS PERSONALIDADES 🎪")

    with gr.Column(elem_classes=["circus-card"], visible=True) as paso1_registro:
        gr.Markdown("### 📝 Registro del Candidato")
        with gr.Row():
            input_nombre = gr.Textbox(label="Nombre", placeholder="Ej. Carl Gustav")
            input_apellido = gr.Textbox(label="Apellido", placeholder="Ej. Jung")
            input_pref = gr.Textbox(label="Nombre Preferido", placeholder="Ej. Carl")
        
        with gr.Row():
            input_telefono = gr.Textbox(label="Teléfono (Pi)", placeholder="Ej. 3141592653")
            input_correo = gr.Textbox(label="Correo Electrónico", placeholder="Ej. carlgustav.jung@jungtech.com")
        
        lbl_error = gr.Markdown("")
        btn_entrar = gr.Button("🎪 Entrar a la Carpa Digital 🎪", elem_classes=["circus-btn-main"])

    with gr.Column(elem_classes=["circus-card"], visible=False) as paso2_banners:
        lbl_instrucciones = gr.Markdown("")
        
        with gr.Row(elem_classes=["grid-banners"]):
            for char in [
                {"nombre": "Harry Potter", "mbti": "ISFP"},
                {"nombre": "Hermione Granger", "mbti": "ESTJ"},
                {"nombre": "Ron Weasley", "mbti": "ESFP"},
                {"nombre": "Albus Dumbledore", "mbti": "INFJ"},
                {"nombre": "Severus Snape", "mbti": "INTJ"},
                {"nombre": "Luna Lovegood", "mbti": "INFP"},
                {"nombre": "Remus Lupin", "mbti": "INTP"},
                {"nombre": "Sirius Black", "mbti": "ESTP"},
                {"nombre": "Lord Voldemort", "mbti": "ENTJ"},
                {"nombre": "Fred Weasley", "mbti": "ENTP"},
                {"nombre": "Minerva McGonagall", "mbti": "ISTJ"},
                {"nombre": "Rubeus Hagrid", "mbti": "ISFJ"},
                {"nombre": "Draco Malfoy", "mbti": "ISTP"},
                {"nombre": "Ginny Weasley", "mbti": "ESTP"},
                {"nombre": "Bellatrix Lestrange", "mbti": "ESFP"},
                {"nombre": "Neville Longbottom", "mbti": "ISFJ"}
            ]:
                # Botones limpios mostrando únicamente el nombre del personaje sin revelar el MBTI de frente
                btn_banner = gr.Button(
                    value=f"⚡ {char['nombre']}",
                    elem_classes=["banner-btn"]
                )
                btn_banner.click(
                    fn=evaluar_seleccion_web,
                    inputs=[gr.State(char['nombre']), gr.State(char['mbti']), state_usuario],
                    outputs=[lbl_instrucciones, paso2_banners, paso1_registro]
                )

    def iniciar_flujo_web(n, a, pref, tel, c):
        err, p1, p2, msg = validar_registro_web(n, a, c)
        return err, p1, p2, msg, f"{n} {a} ({pref})"

    btn_entrar.click(
        fn=iniciar_flujo_web,
        inputs=[input_nombre, input_apellido, input_pref, input_telefono, input_correo],
        outputs=[lbl_error, paso1_registro, paso2_banners, lbl_instrucciones, state_usuario]
    )


if __name__ == "__main__":
    # Descomenta la siguiente línea si deseas ejecutar la consola CLI original:
    # ejecutar_agente()
    
    # Por defecto se lanza la interfaz web visual con el estilo de Circo Digital:
    demo.launch()